-- MLM Upgrade Migration
-- Date: 2024-03-20
-- Description: Updates database schema for MLM features

-- Add new columns to users table
ALTER TABLE `users`
ADD COLUMN `referral_code` VARCHAR(16) UNIQUE AFTER `phone`,
ADD COLUMN `total_referrals` INT DEFAULT 0 AFTER `referral_code`,
ADD COLUMN `total_qoinz_earned` DECIMAL(10,2) DEFAULT 0.00 AFTER `total_referrals`,
ADD COLUMN `last_level_up` TIMESTAMP NULL AFTER `total_qoinz_earned`;

-- Add new columns to branches table
ALTER TABLE `branches`
ADD COLUMN `max_members` INT DEFAULT 9 AFTER `member_count`,
ADD COLUMN `completion_rewards` JSON NULL AFTER `completion_date`,
ADD COLUMN `auto_placement_enabled` BOOLEAN DEFAULT FALSE AFTER `completion_rewards`;
...
ADD FOREIGN KEY (`category_id`) REFERENCES `achievement_categories` (`id`);
...
DELIMITER //
CREATE PROCEDURE `check_level_up`(IN `user_id` INT)
BEGIN
...
DELIMITER //
CREATE PROCEDURE `check_branch_completion`(IN `branch_id` INT)
BEGIN
